...
// 2. Nav links render
const navLinks = await page.$$eval('nav button, nav a', els => els.map(e => e.textContent?.trim()).filter(Boolean).slice(0, 10));
...
if (opts2.length) { await opts2[0].click(); await page.click('text=Continue'); await page.waitForTimeout(400); }
// Step 3
const opts3 = await page.$$('button[style*="rgba(59,130,246,0.04)"]');
if (opts3.length) { await opts3[0].click(); await page.click('text=Continue'); await page.waitForTimeout(400); }
// Step 4
const opts4 = await page.$$('button[style*="rgba(59,130,246,0.04)"]');
if (opts4.length) { await opts4[0].click(); await page.click('text=Generate My Learning Plan'); await page.waitForTimeout(400); }

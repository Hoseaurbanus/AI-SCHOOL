...
export default function AdminCertificates() {
  ...
  const stats = useMemo(() => {
    ...
    const month = certificates.filter(c => c.date.startsWith('2026-07')).length
  ...
  const filtered = useMemo(() => {
    ...
    return certificates.filter(cert => {
      const matchSearch = cert.studentName.toLowerCase().includes(search.toLowerCase()) || cert.id.toLowerCase().includes(search.toLowerCase())
  ...
  if (isLoading) {
    return (
      <div className="p-4 sm:p-6 lg:p-8 flex items-center justify-center min-h-[400px]">
        <div className="text-sm" style={{ color: '#64748B' }}>Loading certificates...</div>
      </div>
    )
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: '#64748B' }}>{stats.total} certificates issued</p>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-4 mb-6">
        <StatsCard icon={Award} title="Total Issued" value={stats.total} color="#F59E0B" />
        <StatsCard icon={Award} title="This Month" value={stats.month} color="#3B82F6" />
        <StatsCard icon={Award} title="Pending" value={stats.pending} color="#F59E0B" />
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl" style={{ background: '#0D1421', border: '1px solid rgba(239,68,68,0.1)', width: '240px' }}>
          ...
          <input type="text" placeholder="Search certificates..." value={search} onChange={e => setSearch(e.target.value)} className="bg-transparent text-sm outline-none flex-1" style={{ color: '#F1F5F9' }} />
        </div>
        <div className="flex gap-1 p-1 rounded-xl" style={{ background: 'rgba(239,68,68,0.05)' }}>
          {['all', 'issued', 'pending'].map(f => (
            <button key={f} onClick={() => setStatusFilter(f)} className="px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-all" style={{ background: statusFilter === f ? 'rgba(239,68,68,0.15)' : 'transparent', color: statusFilter === f ? '#EF4444' : '#64748B' }}>
              {f}
            </button>
          ))}
        </div>
          ...
          <tbody>
            {filtered.map(cert => (
              <tr key={cert.id} className="border-b" style={{ borderColor: 'rgba(239,68,68,0.04)' }}>
                ...
                <td className="px-5 py-4 text-sm" style={{ color: '#F1F5F9' }}>{cert.studentName}</td>
                <td className="px-5 py-4 text-xs" style={{ color: '#94A3B8' }}>{cert.courseName}</td>

...
export default function AdminUsers() {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: 'Courses',
      render: (user: any) => (
        <span className="text-sm" style={{ color: '#94A3B8' }}>{user.courses}</span>
      )
    },
    {
      key: 'progress',
    ...
    },
    {
      ...
      label: 'Joined',
      render: (user: any) => (
        <span className="text-xs" style={{ color: '#64748B' }}>{user.joined}</span>
      )
    },
    {
      key: 'role',
      ...
      render: (user: any) => (
        <div className="flex gap-2">
          ...
          <button 
            ...
            style={{ color: '#475569' }} 
            onMouseEnter={e => (e.currentTarget.style.color = user.status === 'active' ? '#EF4444' : '#10B981')} 
            onMouseLeave={e => (e.currentTarget.style.color = '#475569')}
            onClick={() => user.status === 'active' ? setSuspendId(user.id) : showToast('User already active', 'info')}
          >

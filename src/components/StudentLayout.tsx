...
import {
  LayoutDashboard, BookOpen, MessageSquare, Code2, ClipboardList,
  FolderKanban, Award, User, Settings, Bell, LogOut, Zap, Menu, X,
  ChevronRight,
} from 'lucide-react'
...
const navItems = [
  { icon: LayoutDashboard, label: 'Dashboard', page: 'student-dashboard' as Page },
...
export default function StudentLayout({ children, navigate, currentPage, onLogout }: Props) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        <p className="text-xs font-semibold uppercase tracking-widest px-3 pb-2" style={{ color: '#475569' }}>
          Learning
        </p>
        {navItems.map(({ icon: Icon, label, page }) => {
          ...
          return (
            ...
            >
              ...
              {active && <ChevronRight size={14} style={{ color: '#3B82F6' }} />}
            </button>
          )
        })}
      </nav>

      <div className="px-3 py-3 border-t space-y-0.5" style={{ borderColor: 'rgba(59,130,246,0.1)' }}>
        {bottomItems.map(({ icon: Icon, label, page }) => {
          ...
          return (
            <button
              ...
              style={{ background: active ? 'rgba(59,130,246,0.12)' : 'transparent', color: active ? '#3B82F6' : '#94A3B8' }}
            >
              <Icon size={18} />
              <span>{label}</span>
            </button>
        ...
        <div className="rounded-xl p-3" style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.15)' }}>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-8 h-8 rounded-full overflow-hidden">
              <img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=80&h=80&fit=crop&auto=format" alt="Student" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-medium truncate" style={{ color: '#F1F5F9' }}>Emeka Okafor</p>
              <p className="text-xs truncate" style={{ color: '#64748B' }}>Scholar Plan</p>
            </div>
          </div>
          <div className="space-y-1">
            ...
            <div className="h-1.5 rounded-full" style={{ background: 'rgba(59,130,246,0.15)' }}>
              <div className="h-full rounded-full gradient-blue-purple" style={{ width: '56%' }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        >
          ...
          <div className="flex-1 lg:max-w-xs">
            ...
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="#475569" strokeWidth={2}>

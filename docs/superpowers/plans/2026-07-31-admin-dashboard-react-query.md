# AdminDashboard React Query Enhancement Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- []`) syntax for tracking.

**Goal:** Rewrite AdminDashboard.tsx to use React Query hooks (`useAdminStats`, `useAdminTransactions`) and the shared `StatsCard` component, replacing hardcoded mock data imports with hook-based data fetching.

**Architecture:** Single-file rewrite of `src/pages/AdminDashboard.tsx`. Remove direct `adminStats` and `recentTransactions` imports, replace with React Query hooks. Replace local `StatCard` with imported `StatsCard` component. Preserve all existing UI layout and functionality.

**Tech Stack:** React 19, React Query (TanStack Query), StatsCard component, Lucide icons

## Global Constraints

- Use existing project patterns and conventions
- Maintain all existing UI functionality (enrollment chart, quick actions, transactions table)
- Preserve dark theme styling with inline styles
...
**Interfaces:**
- Consumes: `useAdminStats` from `../hooks/useAdmin`, `useAdminTransactions` from `../hooks/useAdmin`, `StatsCard` from `../components/admin/StatsCard`
- Produces: Enhanced `AdminDashboard` component with React Query data fetching

- [ ] **Step 1: Rewrite AdminDashboard.tsx with React Query hooks and StatsCard**

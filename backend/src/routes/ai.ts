...
const chatSchema = z.object({
  ...
  agentType: z.enum(['tutor', 'mentor', 'coder', 'assessor', 'coach']).default('tutor'),
});
...
export async function aiRoutes(app: FastifyInstance) {
  ...
  app.post('/chat', async (request, reply) => {
    ...
    try {
      ...
      reply.raw.write(`data: ${JSON.stringify({ error: true, message: 'AI service unavailable' })}\n\n`);
      reply.raw.end();
    }
  });
  ...
  app.get('/recommendations', async (request, reply) => {
    ...
    const recommendations = [
      { type: 'course', title: 'Recommended Course', reason: 'Based on your interests' },
      { type: 'lesson', title: 'Review Lesson', reason: 'You struggled with this topic' },
      { type: 'practice', title: 'Practice Exercise', reason: 'Strengthen your skills' },
    ];

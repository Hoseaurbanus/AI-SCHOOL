...
export async function assessmentRoutes(app: FastifyInstance) {
  ...
  app.get('/:id', async (request, reply) => {
    ...
    const [assessment] = await app.db.select().from(assessments)
      .where(eq(assessments.id, id))
      .limit(1);
    
    if (!assessment) {
      return reply.status(404).send({ error: true, message: 'Assessment not found' });
    }
  ...
  app.post('/:id/start', async (request, reply) => {
    ...
    const previousAttempts = await app.db.select().from(assessmentResults)
      .where(and(
  ...
  app.post('/:id/submit', async (request, reply) => {
    ...
    // Save result
    const [result] = await app.db.insert(assessmentResults).values({
      assessmentId: id,
      userId,
  ...
  app.get('/me/results', async (request, reply) => {
    ...
    const results = await app.db.select().from(assessmentResults)
